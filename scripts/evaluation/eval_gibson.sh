#!/bin/bash
##################################################
### This script is to evaluate the full HERE system 
...
### Select scenes
##################################################
scenes=( Cantwell Denmark Eastville Elmira Eudora Greigsville Pablo Ribera Swormville ) #( Cantwell Denmark Eastville Elmira Eudora Greigsville Pablo Ribera Swormville )
# Check if the input argument is 'all'
if [ "$scene" == "all" ]; then
...
do
    for i in $selected_trials; do
        ...
        GT_MESH=$PROJ_DIR/data/gibson/${scene}.glb
        ...
        python src/evaluation/eval_uq_v2.py \
            --cfg configs/${DATASET}/${scene}/${EXP}.py \
            --ckpt_dir ${result_dir}/coslam/checkpoint \

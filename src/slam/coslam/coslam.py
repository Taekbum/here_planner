"""
We have reused part of CoSLAM's code in this file and include our code for HERE.
...
from src.slam.coslam.coslam_utils import query_point_sdf, get_map_volumes, entropy, trimesh_to_open3d, calc_ause, plot_errors
from src.slam.slam_model import SlamModel

### modified coslam modules ###
from src.slam.coslam.model.scene_rep import JointEncodingHere as JointEncoding
from src.slam.coslam.model.keyframe import KeyFrameDatabaseHere as KeyFrameDatabase
...
class CoSLAMHere(SlamModel, CoSLAM):
    def __init__(self, 
        ...
        if self.config['decoder']['uncert_grid']:
            ...
            self.grid_type = 'uncert'
        elif self.config['decoder']['prior_grid']:
            self.init_prior_grid_optim(voxel_size=self.voxel_size)
            self.grid_type = 'prior'
        ...
        self.initial_uncert = self.get_initial_uncert()
    
    def get_initial_uncert(self):
        raw_prob = -self.model.log_scale * torch.ones(1)
        prob = self.model.act_raw_prob(raw_prob)
        evidence = np.exp(self.model.log_scale) * prob
        
        raw_inv_prec = 3 * torch.ones(1)
        z = self.model.act_uncertainty(raw_inv_prec) + 0.01
        
        posterior_evidence = evidence + self.model.prior_evidence
        posterior_z = (z * evidence + self.model.prior_z * self.model.prior_evidence) / posterior_evidence
        
        alpha = 0.5 * posterior_evidence
        beta = 0.5 * posterior_evidence * posterior_z
        uncert = F.softplus(entropy(alpha, beta, 2*alpha)) + 0.01
        return uncert[0].item() - 0.0001
    ...
    def get_loss_from_ret(self, ret, rgb=True, sdf=True, depth=True, fs=True, uncert=True, smooth=False):
        ...
        elif uncert and self.config['decoder']['prior_grid']:
            if sdf:
                loss += self.config['training']['sdf_uncert_weight'] * ret['sdf_uncert_loss']
            if fs:
                loss += self.config['training']['fs_uncert_weight'] * ret['fs_uncert_loss']
    ...
    def first_frame_mapping(self, batch, n_iters=100):
        ...
        if self.config['decoder']['uncert_grid']:
            ...
            self.evidence_optim.zero_grad()
            self.precision_optim.zero_grad()
        ...
        if self.config['decoder']['uncert_grid']:
            ...
            self.evidence_optim.step()
            self.precision_optim.step()
    ...
    def init_prior_grid_optim(self, voxel_size = 0.1):
        """ Initialzie Prior Grid optimizer
        """
        self.evidence_optim = torch.optim.Adam(params=[self.model.get_evidence_grid(voxel_size)], lr=self.config['mapping']['lr_evid'])
        self.precision_optim = torch.optim.Adam(params=[self.model.get_precision_grid(voxel_size)], lr=self.config['mapping']['lr_prec'])
    ...
    def global_BA(self, batch, cur_frame_id):
        ...
        for i in range(self.config['mapping']['iters']):
            ...
            # set depth from far bound for active ray sampling of invalid depth # 
            target_d_for_sample = target_d.clone()
            valid_depth_mask = ((target_d_for_sample > 0) & (target_d_for_sample <= self.config["cam"]["depth_trunc"])).squeeze()
            if self.config['training'].get('train_missing_depth', False) and (~valid_depth_mask).sum() > 0:
                det_rays_o = rays_o[~valid_depth_mask].clone().detach().unsqueeze(-1)   # (N, 3, 1)
                det_rays_d = rays_d[~valid_depth_mask].clone().detach().unsqueeze(-1)  # (N, 3, 1)
                t = (self.model.far_bound.unsqueeze(0).cuda() - det_rays_o)/det_rays_d  # (N, 3, 2)
                target_d_invalid, _ = torch.min(torch.max(t, dim=2)[0], dim=1)
                target_d_for_sample[~valid_depth_mask] = target_d_invalid.unsqueeze(-1)
            ...
            elif self.config['decoder']['prior_grid'] and (i + 1) % 5 == 0:
                self.evidence_optim.step()
                self.precision_optim.step()
                self.evidence_optim.zero_grad()
                self.precision_optim.zero_grad()
    ...
    @torch.no_grad()
    def eval_uq_v2(self, step, mesh_gt, mesh_rec, output_dir, vis=None, sdf_err=True):
        import trimesh
        ...
        vert_flat = torch.from_numpy(gt_pc_tri.vertices).float()
        bounding_box_cpu = self.bounding_box.cpu()
        if self.config['grid']['tcnn_encoding']:
            vert_flat = (vert_flat - bounding_box_cpu[:, 0]) / (bounding_box_cpu[:, 1] - bounding_box_cpu[:, 0])
        
        chunk = 1024 * 64
        fn_uncert = lambda f, i0, i1: self.model.query_sdf(f[i0:i1, None, :].to(self.bounding_box.device), return_uncert=True)[:, 0].cpu() # query function for uncertainty
        pred = [fn_uncert(vert_flat,  i, i + chunk) for i in range(0, vert_flat.shape[0], chunk)]
        pred = torch.cat(pred, 0)
        if self.config['decoder']['uncert_grid']:
            sdf, uncert_map_tensor = pred[:, 0], pred[:, 1]
        elif self.config['decoder']['prior_grid']:
            sdf, alpha, beta = pred[:, 0], pred[:, 1], pred[:, 2]
            uncert_map_tensor = entropy(alpha, beta, 2*alpha)
        uncert_map = uncert_map_tensor.numpy()

        if sdf_err:
            distances_tensor = sdf.abs()
            distances = distances_tensor.numpy()
        else: # vertice completion error
            rec_pc = trimesh.sample.sample_surface(mesh_rec, 200000)
            rec_pc_tri = trimesh.PointCloud(vertices=rec_pc[0])
            rec_points_kd_tree = KDTree(rec_pc_tri.vertices)
            distances, _ = rec_points_kd_tree.query(gt_pc_tri.vertices)
            distances_tensor = torch.from_numpy(distances).float()
        
        output_path = os.path.join(output_dir, 'ause_3d_plot')
        os.makedirs(output_path, exist_ok=True)
        ratio, err, err_var, ause = calc_ause(uncert_map_tensor, distances_tensor, err_type='rmse')
        plot_errors(ratio, err, err_var, 'rmse', step, output_path)
        self.metric_ause_3d.append(ause)

        if vis is not None:
            # err_map, _ = rec_points_kd_tree.query(mesh_gt.vertices)
            # err_map_max = np.percentile(err_map, 95)
            # err_map_min = np.percentile(err_map, 5)
            # err_map = np.clip(err_map, err_map_min, err_map_max)

            vert_flat = torch.from_numpy(mesh_gt.vertices).float()
            if self.config['grid']['tcnn_encoding']:
                vert_flat = (vert_flat - bounding_box_cpu[:, 0]) / (bounding_box_cpu[:, 1] - bounding_box_cpu[:, 0])
            ...
            uncert_map = uncert_map_tensor.numpy()
            uncert_map_max = np.percentile(uncert_map, 95)
            uncert_map_min = np.percentile(uncert_map, 5)
            uncert_map = np.clip(uncert_map, uncert_map_min, uncert_map_max)

            err_map = sdf.abs().numpy()
            err_map_max = 1 #err_map.max() #np.percentile(err_map, 95)
            err_map_min = err_map.min() #np.percentile(err_map, 5)

            colormap = plt.get_cmap('jet')
            uncert_normalized = np.clip(uncert_map - uncert_map_min, 1e-4, None) / np.clip(uncert_map_max - uncert_map_min, 1e-4, None) # To avoid strange unc color in step 0
            uncert_color = colormap(uncert_normalized.flatten())[:, :3]  # Discard alpha channel
            gt_mesh_unc = trimesh.Trimesh(mesh_gt.vertices, mesh_gt.faces, process=False, vertex_colors=uncert_color)
            gt_mesh_unc_o3d = trimesh_to_open3d(gt_mesh_unc)

            err_normalized = np.clip(err_map - err_map_min, 1e-4, None) / np.clip(err_map_max - err_map_min, 1e-4, None)
            err_color = colormap(err_normalized.flatten())[:, :3]
            gt_mesh_err = trimesh.Trimesh(mesh_gt.vertices, mesh_gt.faces, process=False, vertex_colors=err_color)
            gt_mesh_err_o3d = trimesh_to_open3d(gt_mesh_err)
            
            # render uncertainty of gt vertices
            render_filepath_unc = os.path.join(output_path, f"uncert_gt_mesh_{step:04}.png")
            vis.clear_geometries()
            vis.add_geometry(gt_mesh_unc_o3d)
            vis.poll_events()
            vis.update_renderer()
            vis.capture_screen_image(render_filepath_unc, do_render=True)
            
            # render completion error of gt vertices
            render_filepath_err = os.path.join(output_path, f"err_gt_mesh_{step:04}.png")
            vis.clear_geometries()
            vis.add_geometry(gt_mesh_err_o3d)
            vis.poll_events()
            vis.update_renderer()
            vis.capture_screen_image(render_filepath_err, do_render=True)
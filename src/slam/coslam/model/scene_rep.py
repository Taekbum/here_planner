...
from src.slam.coslam.model.decoder import ColorSDFNet_v2_Here as ColorSDFNet_v2
from src.slam.coslam.coslam_utils import get_sdf_evidential_loss, clamp_preserve_gradients
...
class JointEncodingHere(JointEncoding):
    def __init__(self, config, bound_box):
        ...
        if self.config['decoder']['pred_uncert'] or self.config['decoder']['uncert_grid']:
            ...
            self.act_uncertainty = nn.Softplus()
            self.act_raw_prob = nn.Sigmoid()
            self.log_scale = self.config['grid']['hash_size']
            self.prior_evidence = 1
            self.prior_z = 3
            
        if self.config['training'].get('train_missing_depth', False):
            margin = 0.1#0.05
            self.far_bound = torch.stack([bound_box[:,0] - margin, bound_box[:,1] + margin], dim=-1)
    ...
    def get_evidence_grid(self, voxel_size): # NOTE: when using this with sigmoid, scale by 100 first ! -> to make initial prob 0
        Nx = round((self.bounding_box[0, 1] - self.bounding_box[0, 0]).item() / voxel_size + 0.0005) + 1
        Ny = round((self.bounding_box[1, 1] - self.bounding_box[1, 0]).item() / voxel_size + 0.0005) + 1
        Nz = round((self.bounding_box[2, 1] - self.bounding_box[2, 0]).item() / voxel_size + 0.0005) + 1
        # raw probability, log of evidence (log(n)=log(2*alpha)) initialize to 0
        self.evidence_grid = torch.nn.parameter.Parameter(-torch.ones([Nx, Ny, Nz], device="cuda").float() * self.log_scale)  # logit for probability (by sigmoid) (initialized by 0) 
        self.cache_evidence = np.zeros([Nx, Ny, Nz], dtype=np.float32)
        return self.evidence_grid
    
    def get_precision_grid(self, voxel_size):
        Nx = round((self.bounding_box[0, 1] - self.bounding_box[0, 0]).item() / voxel_size + 0.0005) + 1
        Ny = round((self.bounding_box[1, 1] - self.bounding_box[1, 0]).item() / voxel_size + 0.0005) + 1
        Nz = round((self.bounding_box[2, 1] - self.bounding_box[2, 0]).item() / voxel_size + 0.0005) + 1
        # log of precision (log(n/(2*beta))) initialize to log(1/3) 3 <-> get_uncert_grid's initial value 
        self.precision_grid = torch.nn.parameter.Parameter(torch.ones([Nx, Ny, Nz], device="cuda").float() * 3)
        self.cache_precision = np.zeros([Nx, Ny, Nz], dtype=np.float32)
        return self.precision_grid
    ...
    def calc_update(self, inputs): # mu
        # padding_mode='border' To avoid getting unwanted zero for raw_inv_prec & raw_prob when input escapes scene boundary
        # (especially, raw_prob=0 makes evidence very big!)
        grid = (inputs * 2 - 1)[None, None, None, ...]
        raw_inv_prec = torch.nn.functional.grid_sample(self.precision_grid[None, None, ...], grid, align_corners=False, padding_mode='border')
        raw_prob = torch.nn.functional.grid_sample(self.evidence_grid[None, None, ...], grid, align_corners=False, padding_mode='border')
        inv_prec = self.act_uncertainty(raw_inv_prec) + 0.01
        prob = self.act_raw_prob(raw_prob)
        evidence = np.exp(self.log_scale) * prob
        evidence = clamp_preserve_gradients(evidence, lower=np.exp(-30), upper=np.exp(30))
        inv_prec = inv_prec.squeeze()[..., None]
        evidence = evidence.squeeze()[..., None]
        
        update_suff_stats = inv_prec# + mu**2 
        
        return update_suff_stats, evidence

    def update(self, update_suff_stats, evidence): # mu
        z = update_suff_stats
        
        posterior_evidence = evidence + self.prior_evidence
        posterior_z = (z * evidence + self.prior_z * self.prior_evidence) / posterior_evidence
        
        alpha = 0.5 * posterior_evidence
        beta = 0.5 * posterior_evidence * posterior_z#(posterior_z - mu ** 2)
    ...
    def query_sdf(self, query_points, return_geo=False, embed=False, return_uncert=False):
        ...
        if self.config['decoder']['prior_grid']:
            update_suff_stats, evidence = self.calc_update(inputs_flat)
            alpha, beta = self.update(update_suff_stats, evidence)
        ...
        if return_uncert and self.config['decoder']['uncert_grid']:
            uncert = torch.reshape(uncert, list(query_points.shape[:-1]))
            sdf = torch.stack([sdf, uncert], -1)  # stack uncertainty to the sdf.
        elif return_uncert and self.config['decoder']['prior_grid']:
            alpha = torch.reshape(alpha, list(query_points.shape[:-1]))
            beta = torch.reshape(beta, list(query_points.shape[:-1]))
            sdf = torch.stack([sdf, alpha, beta], -1)
    ...
    def query_color_sdf(self, query_points):
        ...
        if self.config['decoder']['prior_grid']:
            update_suff_stats, evidence = self.calc_update(inputs_flat)
            alpha, beta = self.update(update_suff_stats, evidence)
            return torch.cat([self.decoder(embed, embe_pos), alpha, beta], -1)
        return self.decoder(embed, embe_pos)
    
    def render_rays(self, rays_o, rays_d, target_d=None, return_weights=False):
        '''
        Params:
            rays_o: [N_rays, 3]
            rays_d: [N_rays, 3]
    ...
    def forward(self, rays_o, rays_d, target_rgb, target_d, global_step=0):
        ...
        if self.config['decoder']['prior_grid']:
            alpha, beta = rend_dict['raw'][..., 4], rend_dict['raw'][..., 5]
            if self.config['training'].get('train_missing_depth', False) and valid_depth_mask.sum() < len(valid_depth_mask):
                target_d_ = target_d.clone()
                target_d_[~valid_depth_mask] = rend_dict["depth"].unsqueeze(-1)[~valid_depth_mask]
                sdf[~valid_depth_mask] = sdf[~valid_depth_mask].detach()
                fs_nll_loss, fs_reg_loss, sdf_nll_loss, sdf_reg_loss = get_sdf_evidential_loss(z_vals, target_d_, sdf, truncation, alpha, beta)
            else:
                fs_nll_loss, fs_reg_loss, sdf_nll_loss, sdf_reg_loss = get_sdf_evidential_loss(z_vals, target_d, sdf, truncation, alpha, beta)
            ret['fs_nll_loss'] = fs_nll_loss
            ret['fs_reg_loss'] = fs_reg_loss
            ret['sdf_nll_loss'] = sdf_nll_loss
            ret['sdf_reg_loss'] = sdf_reg_loss
            ret['sdf_uncert_loss'] = fs_nll_loss-self.config['training']['entropy_weight']*fs_reg_loss
            ret['fs_uncert_loss'] = sdf_nll_loss-self.config['training']['entropy_weight']*sdf_reg_loss
